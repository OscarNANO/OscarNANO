...
def knapsack(peso_permitido, pesos, valores):
    ...
    peso_total = sum(pesos) #cual es el peso total 
...
def poblacion_inicial(n, tam):  #la poblacion inicial
    pobl = np.zeros((tam, n))
    for i in range(tam):
        pobl[i] = (np.round(np.random.uniform(size = n))).astype(int)
    return pobl
 
def mutacion(sol, n):  #mutacion del individuo
    pos = randint(0, n - 1)
    mut = np.copy(sol)
...
n = 400 #numero de objetos a variar
...
plt.savefig('tareadiez400.png', bbox_inches='tight') 
plt.close()
