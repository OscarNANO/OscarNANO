...
library(lattice)
g = function(x, y) {
return (((x + 0.7)^4 - 60 * x^2 - 20 * x + (y + 0.7)^4 - 60 * y^2 - 20 * y)/100)   #funcion cambiada
}
...
names(d) = c("x", "y", "z")
if(j < 10){
nombre =  paste0("tarea7sim_100", j, ".png", sep="")
}else if(j>= 10 & j < 100){ 
nombre =  paste0("tarea7sim_10", j, ".png", sep="") }else{
nombre =  paste0("tarea7sim_1", j, ".png", sep="")
}

png("tareasiete_2d.png", width=700, height=700)
persp(x, y, z, shade=0.2, col='red', theta=40, phi=30)
graphics.off()
